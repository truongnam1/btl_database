<?php
    function listDish(){
        ...
        //lay ma thuc don
        $list_thuc_don = array();
        $sql = "SELECT DISTINCT idthuc_don,ten_thuc_don FROM thuc_don";
        ...
        foreach($list_thuc_don as $ten => $id) {
            $mon_an = array();
            $sql = "SELECT idmon_an, gia, mo_ta, ten_mon_an FROM mon_an WHERE idthuc_don = $id";
            $result = $conn->query($sql);
            while($row = $result->fetch_assoc()) {
                $detail_mon_an = array();
                $detail_mon_an['id'] = $row["idmon_an"];
