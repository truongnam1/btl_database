...
minPrice = elementPriceRange.querySelector('#minPrice').value == null ? "" : elementPriceRange.querySelector('#minPrice').value;
maxPrice = elementPriceRange.querySelector('#maxPrice').value == null ? "" : elementPriceRange.querySelector('#maxPrice').value;
...
elementConceptType.querySelectorAll('input').forEach(eleInput => {
    eleInput.addEventListener("change", function() {
        ...
        } else if (eleInput.checked && indexName >= 0) { // nếu có và cần sửa lại value
            console.log('sua cu ' + conceptSelected + "    moi: " + eleInput.value);
            arrayName[indexName] = arrayName[indexName].replace(conceptSelected, eleInput.value);
        }
...
elementComboBuffet.querySelectorAll('input').forEach(eleInput => {
    eleInput.addEventListener("change", function() {
        ...
        if (eleInput.checked && indexName < 0) {
            ...
            arrayName[indexName] = arrayName[indexName].replace(typeBuffet, eleInput.value);
        }
        showURL(arrayName);
    });
});
...
elementPriceRange.querySelector('button').onclick = function() {
    ...
    if (indexMinPrice < 0 && !(eleMinPrice.value == '')) {
        ...
        arrayName[indexMinPrice] = arrayName[indexMinPrice].replace(minPrice, eleMinPrice.value);
    }
    // showURL(arrayName);
    ...
    if (indexMaxPrice < 0 && !(eleMaxPrice.value == '')) {
        ...
        arrayName[indexMaxPrice] = arrayName[indexMaxPrice].replace(maxPrice, eleMaxPrice.value);
    }
    showURL(arrayName);
}